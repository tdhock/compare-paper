...
\begin{abstract}
  ...
  can be solved by converting it to a standard SVM. We compare our
  algorithm to SVMrank in simulated nonlinear patterns.
\end{abstract}
...
\begin{equation}
  ...
  y_i =
  \begin{cases}
    -1 & \text{ if $\mathbf x_i$ is better than $\mathbf x'_i$},\\
    0 & \text{ if $\mathbf x_i$ is as good as $\mathbf x'_i$},\\
    1 & \text{ if $\mathbf x'_i$ is better than $\mathbf x_i$}.
  \end{cases}
\end{equation}
...
and bold lowercase letters for their row vectors $\mathbf x_i,
\mathbf k_i$. In Section~\ref{sec:related} we discuss links with
related work on classification and ranking, then in
Section~\ref{sec:svm-compare} we propose a new algorithm:
\SVMcompare. We show results on simulated data in
Section~\ref{sec:results} and discuss future work in
...
documents $\mathbf x_i,\mathbf x_i'$ for the same search query. The
labels are $y_i\in\{-1,1\}$, where $y_i=1$ means document $\mathbf
x_i'$ is more relevant than $\mathbf x_i$ and $y_i=-1$ means the
opposite. The SVMrank algorithm was proposed for this problem
\citep{ranksvm}, and the algorithm we propose here is similar. The
difference is that we also consider the case where both
items/documents are judged to be equally good ($y_i=0$).  A boosting
algorithm has been proposed for this ``ranking with ties'' problem
\citep{rank-with-ties}, and the authors observed that modeling ties is
more effective when there are more output values. A Bayesian model
which can be applied to these data is TrueSkill \citep{trueskill}, a
generalization of the Elo chess rating system.
...
inputs $(\mathbf x_i, \mathbf x_i', y_i)$ but instead single inputs
$\mathbf x_i$ with ordinal labels $y_i\in\{1,\dots,k\}$, where $k$ is
the number of integral output values. In this case algorithms such as
Support Vector Ordinal Regression are applicable
\citep{ordinal}. However, in this paper we consider a different
learning problem, where we only have weak labels $y_i\in\{-1,0,1\}$
for pairs $\mathbf x_i,\mathbf x_i'$ of inputs.
...
\begin{figure*}[b!]
  ...
  \caption{The separable LP and QP comparison problems. \textbf{Left}:
    ...
    (\ref{eq:max-margin-qp}). \textbf{Right}: in these scaled data,
...
we extend SVMrank by we defining a threshold $\tau\in\RR^+$ and a
...
Using the following linear program, we learn a linear ranking function
$r(\mathbf x)=\mathbf w^\intercal \mathbf x$ that maximizes the
geometric margin $\mu$. As shown in the left panel of
Figure~\ref{fig:hard-margin}, the geometric margin $\mu$ is the
smallest distance from any difference vector $\mathbf x_i'-\mathbf
x_i$ to a decision boundary $r(\mathbf x)\in\{-1,1\}$. The max margin
LP is
\begin{eqnarray}
...
x)=\beta+\mathbf u^\intercal \mathbf x$ using a binary SVM QP (black
lines in middle and right panels of Figure~\ref{fig:hard-margin}):
\begin{eqnarray}
...
\end{eqnarray}
Intuitively, the SVM QP learns a separator $f(\mathbf x)=0$ between
significant difference pairs $\tilde y_i=1$ and insignificant
difference pairs $\tilde y_i=-1$. However, we want a comparison
function that predicts $c(\mathbf x,\mathbf x')\in\{-1,0,1\}$. So we
use the next Lemma to construct a ranking function $r(\mathbf x)=
\mathbf{\hat w}^\intercal \mathbf x$ that is feasible for the original
max margin comparison LP (\ref{eq:max-margin-lp}), and can be used
with the comparison function $c_1$ (\ref{eq:compare_general}).
\begin{lemma}
...
\begin{proof}
  ...
  \begin{equation}
    ...
    r(\mathbf x) = \mathbf{\hat w}^\intercal x = 1+\hat\mu\text{ and } 
...
I_m]^\intercal\in\{-1,0,1\}^{2m\times m}$. Solving $\nabla_a \mathcal
L=0$ results in the following stationary condition:
\begin{equation}
...
\begin{algorithmic}
  ...
  \STATE {\bfseries Output:} Support vectors $\mathbf{\tilde
    X}_{\textbf{sv}},\mathbf{\tilde X}_{\textbf{sv}}'$, labels
  $\mathbf{\tilde y}_{\textbf{sv}}$, bias~$\beta$, dual variables $\mathbf v$.
...
optimal $\mathbf v$ is sparse. Let $\textbf{sv}=\{i: v_i > 0\}$ be the indices
...
\item[rank] first ignores the equality pairs for learning SVMrank,
  using only $|\mathcal I_1|+|\mathcal I_{-1}|$ pairs
  (Section~\ref{sec:svmrank}). Then we use all $n$ pairs to learn a
  threshold $\hat \tau$ via grid search for when to predict
  $c(\mathbf x,\mathbf x')=0$.
...
test error by learning from the equality $y_i=0$ pairs.  We generated
pairs $\mathbf x_i,\mathbf x_i'\in[-3,3]^2$ and noisy labels
...
\begin{figure*}[b!]
  ...
  \vskip -1cm
  \caption{Test error for 3 different simulated patterns $r(\mathbf x)$ where
    $\mathbf x\in\RR^2$. We randomly generated data sets with half equality
...
\begin{figure*}[b!]
  ...
  \caption{Area under the ROC curve (AUC) for 3 different simulated
    patterns $r(\mathbf x)$ where $\mathbf x\in\RR^2$. In each
    simulation we generated $n=400$ pairs, varying the proportion
    $\rho$ of equality pairs. We plot mean and standard deviation of
    AUC over 4 test sets.}
  \label{fig:auc}
\end{figure*}
