...
\begin{abstract}
  ...
  nonlinear patterns to show that our proposed SVMcompare algorithm
  outperforms SVMrank when there are equality $y=0$ pairs.
\end{abstract}
...
The goal of SVMrank is to learn a ranking function
...
labeled pairs (\ref{eq:min_c}). We test the SVMcompare algorithm
alongside two baseline models that use SVMrank \citep{ranksvm}. The
differences between these algorithms are summarized in
...
observed in learning to rank data sets. For scaling to these very
large data sets, we would like to try algorithms based on smooth
...
\begin{figure*}[b!]
  ...
  \caption{Test error for 3 different simulated patterns $r(\mathbf
    x)$ where $\mathbf x\in\RR^2$. We randomly generated data sets
