...
(May-Dec). We then computed the test AUC for each of the eight years. Since there are 3 labels $y\in\{-1,0,1\}$ corresponding to $\{$win, draw, loss$\}$ respectively, the baseline AUC is non-standard (0.5).  We calculated the baseline AUC for each dataset by calculating the TPR and FPR by predicting the most common positive class in each dataset.  In our baseline, to maximize the FPR, we set $\tau = 0$ which results in every observation being predicted as a negative class (0).  By predicting every observation as the most common positive class, we have maximized the TPR without learning.  Since we have 2 distinct positive classes, it is highly unlikely that our TPR will be 1.0 through this method.
...
\begin{figure*}[]
	\centering
	\input{chess_fig}	
	\caption{Test AUC for each model used after training on the first 4 months of match data in the 8 different 12-month periods.  All SVM model AUCs are shown in addition to AUC of the ELO, Glicko scores and baseline.  The plots in black show the baseline AUC calculated by predicting the most common label.  The plots in dark blue are AUC values obtained from using Glicko or ELO scores only.  The plots in light blue show the AUC distribution from models using only ELO and Glicko features and plots in white are AUC values from models using all computed features.}
	\label{fig:chess}
\end{figure*}
