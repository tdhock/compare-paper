...
output values. In this case algorithms such as Support Vector Oridinal
...
SVMrank \citep{ranksvm}. This simulation has two goals: (1) visualize
the learned 2D nonlinear ranking functions, and (2) demonstrate that
our model can achieve lower test error by learning from the equality
...
\begin{figure*}[b!]
  ...
  \caption{Test error of SVMrank and SVMcompare (the Bayes error rate
    of the latent $r$ is shown for comparison). We plot mean and
    standard deviation of the prediction error across 4 randomly
    chosen data sets, as a function of percent $y_i=0$ equality pairs
    in data sets of size $n=100$ pairs.}
