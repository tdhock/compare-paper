...
\begin{figure*}[b!]
  ...
  \caption{Application to a simulated pattern $r(x)=||x||_1^2$ where
    ...
    functions. The rank and rank2 models do not directly model the
    equality pairs, so SVMcompare recovers the latent
    pattern better.}
...
\begin{figure*}[b!]
  ...
  \caption{Test error for 3 different simulated patterns $r(x)$ where
    $x\in\RR^2$. We randomly generated 4 data sets with half equality
    and half inequality pairs, then plot mean and standard deviation
    of the test error as a function of training set size $n$ (a
    vertical line shows the data set which was used in
    Figure~\ref{fig:norm-level-curves}).}
  \label{fig:simulation-samples}
\end{figure*}

\begin{figure*}[b!]
  \input{figure-simulation-proportion}
  \vskip -1cm
  \caption{Test error for 3 different simulated patterns $r(x)$ where
    $x\in\RR^2$. In each simulation we generated $n=400$ pairs,
    varying the percent of equality pairs. We plot mean and standard
    deviation of the test error over 4 random seeds.}
  \label{fig:simulation-proportion}
\end{figure*}
