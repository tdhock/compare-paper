...
Comparison is similar to classification with a reject
option and ranking (Table~\ref{tab:related}). The defining features of the
comparison problem that we study in this paper are that inputs are
pairs $\mathbf x,\mathbf x'\in\RR^p$, and outputs $y\in\{-1,0,1\}$
include the $y=0$ equality pairs or ties. 
...
inputs $\mathbf x\in\RR^p$ we consider in this paper. The
classification with reject option problem is a version of binary
classification with outputs $y\in\{-1,0,1\}$, where 0 signifies
