...
$1,2,3,4$.  We then used the selected hyper-parameters to train a
...
\begin{figure*}[!b]
	\centering
	\input{chess_fig}	
	\caption{Test AUC for each model used after training on the first 4 months of match data in the 8 different 12-month periods.  All SVM model AUCs are shown in addition to AUC of the ELO, Glicko scores and baseline.  The plots in light blue show AUC from model using only ELO and Glicko features and plots in dark blue are AUC values from models using all computed features.}
	\label{fig:chess}
\end{figure*}
