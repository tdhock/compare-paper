...
of labeled pairs. We use the zero-one loss for evaluating the learned
comparison function on the test set (Table~\ref{tab:evaluation}).
...
\begin{figure*}[b!]
  ...
  \caption{Application to simulated patterns $x,x'\in\RR^2$, where the
    ...
    and half inequality (red arrows point to the higher rank). which
    are used directly by the comparison model, but are ignored by the
...
function (\ref{eq:threshold}), $r$ is the latent ranking
...
threshold via grid search for when to predict $c(x,x')=0$. In
Figure~\ref{fig:simulation-samples} the test error of the proposed
\proc{SVMcompare} model is clearly lower than that of the baseline
SVMrank model.
...
\begin{figure*}[b!]
  ...
  \vskip -1cm
  \caption{  directly by the comparison model, but are
    ignored by the ranking model (the Bayes error rate of the latent
    $r$ is shown for comparison). We plot mean and standard deviation
    of the prediction error across 4 randomly chosen data sets, as a
    function of training set size $n$. It is clear that the comparison
    model makes better predictions on the test data.}
