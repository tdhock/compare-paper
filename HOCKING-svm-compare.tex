...
(May-Dec). We then computed the test AUC for each of the eight years. Since their are 3 labels $y\in\{-1,0,1\}$ corresponding to $\{$win, draw, loss$\}$ respectively, the baseline AUC is non-standard (0.5).  We calculated the baseline AUC for each dataset by calculating the TPR and FPR by predicting the most common class (a draw in all 8 sets). 
