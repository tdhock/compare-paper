...
parameter_df <- expand.grid(C=10^seq(-20,5, l=26),degree=c(1,2,3,4))

AUC_polydot <- rankSVM_CV(Xi_train = matrix_Xi,Xip_train = matrix_Xip,train_yi = yi$matches.RESULT,parameter.df = parameter_df,kern = polydot,splits_train = c(0.5,0.75,0.80,0.85))
...
qp <- softCompareQP(pairs,kernel = polydot,C=kern.params[1],degree=kern.params[2])
...
out_dir <- paste(myargument,"_polydot_AUC_2ftrs",sep="")
