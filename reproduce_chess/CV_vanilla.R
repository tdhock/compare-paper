...
parameter_df <- expand.grid(C=10^seq(-20, 5, l=26))

AUC_polydot <- rankSVM_CV(Xi_train = matrix_Xi,Xip_train = matrix_Xip,train_yi = yi$matches.RESULT,parameter.df = parameter_df,kern = vanilladot,splits_train = c(0.5,0.75,0.8,0.85))

print("done Vanilla Training")
...
C.para <- AUC_polydot$Group.1[1]

pairs <- list(Xi=matrix_Xi,Xip=matrix_Xip,yi=yi$matches.RESULT)

qp <- softCompareQP(pairs,kernel = vanilladot,C=C.para,tol=0.00001)
...
out_dir <- paste(myargument,"_Vanilla_AUC_aftrs",sep="")
